...
from models.network_modules import ConvBNReLU, ResnetBasicBlock, PositionalEmbedding
...
class TextAdapterDepth(nn.Module):
    def __init__(self, c_in, text_dim=768):
        super().__init__()

        norm_layer = nn.BatchNorm2d
        self.cond_encoder = nn.Sequential(
            ConvBNReLU(C_in=c_in,C_out=128,kernel_size=7, stride=2, norm_layer=norm_layer),
            ConvBNReLU(C_in=128,C_out=128, kernel_size=3,stride=4, norm_layer=norm_layer),
            ConvBNReLU(C_in=128,C_out=text_dim, kernel_size=3,stride=4, norm_layer=norm_layer),
            ResnetBasicBlock(text_dim,text_dim, bias=True, norm_layer=norm_layer),
        )
        self.pos_embed = PositionalEmbedding(d_model=text_dim, max_len=400)

    def forward(self, ref_cond):
        bs, channel, h, w = ref_cond.shape
        ref_cond_feat = self.cond_encoder(ref_cond)
        out = self.pos_embed(ref_cond_feat.reshape(bs, ref_cond_feat.shape[1], -1).permute(0,2,1))
        return out
